...
interface FilterCardProps{
    filterMethod : Record<string, string>[],
    nameForInput : string,
    widthOfLi : string,
      ...
      <ul className={`p-3 pl-4 ${isPending ? 'bg-slate-300/85' : 'bg-slate-100/85'} rounded-md w-full flex flex-col transition-colors `}>
        {filterMethod.map((category, index) => (
          <li key={index} className={`py-3 mb-2 ${widthOfLi} w-40 mx-auto`}>
            <div className={`flex flex-row items-center gap-2 rounded-md p-1  ${usedSortMethod === category.opt ? 'bg-primary-bue text-white' : ''}`}>
              <input type='radio' 
              checked={selectedOption === category.opt}
              onChange={handleChange}
              id={`${category}_${index}`} 
              name={nameForInput} 
              value={category.opt} 
              className='' />
              <label htmlFor={`${category}_${index}`}>{category.label}</label>
            </div>
            </li>
        ))}
      </ul>
